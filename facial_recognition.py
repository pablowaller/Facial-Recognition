...
DETECTION_INTERVAL = 300  # Intervalo mínimo entre detecciones (en segundos)

# Variables globales
last_detection_time = {}
is_processing = False  # Bandera para evitar múltiples ejecuciones simultáneas

# Descargar imágenes de Firebase para reconocimiento facial
...
# Codificar las imágenes descargadas
...
# Analizar la imagen de alta resolución
...
def handle_capture_signal(event):
    ...
    try:
        if event.data and event.data.get("pressed", False) and not is_processing:
            print("🔹 Señal de captura recibida desde Firebase.")
            analyze_high_res_image()
            
            capture_ref.set({"pressed": False}) 
            print("🔄 Nodo de captura restablecido a false.")
    except Exception as e:
        print(f"❌ Error en handle_capture_signal: {e}")


capture_ref = db.reference('capture')
capture_ref.listen(handle_capture_signal)

print("🔹 Esperando señal de captura desde Firebase...")
